                        ...
                        <div class="table-responsive">
                            <table class="table table-hover mt-4">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <img src="{{ asset('uploads/'.$item->img) }}" width="70px"
                                                style="height: 10%">
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <form class="d-inline" action="/increasecart" method="POST">
                                                    @csrf
                                                    <input type="hidden" name="id_user" value="{{ $user->id }}">
                                                    <input type="hidden" name="id" value="{{ $item->id }}">
                                                    <button type="submit"
                                                        class="btn btn-light increaseQuantity">+</button>
                                                </form>
                                                <button
                                                    class="btn btn-light mx-2 quantity">{{ $item->quantity }}</button>
                                                <form class="d-inline" action="/decreasecart" method="POST">
                                                    @csrf
                                                    <input type="hidden" name="id_user" value="{{ $user->id }}">
                                                    <input type="hidden" name="id" value="{{ $item->id }}">
                                                    <button type="submit"
                                                        class="btn btn-light decreaseQuantity">-</button>
                                                </form>
                                        ...
                                        <td>
                                            ...
                                            <p class="text-danger">{{ number_format($item->price, 0, ',', '.') . ' đ' }}
                                            </p>
                                        </td>
                                        <td>
                                            <p class="text-danger subtotal">
                    ...
                    <div class="card-body">
                        ...
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                Tạm tính:
                                <span
                                    class="text-success totalPrice">{{ number_format($totalPrice, 0, ',', '.') . ' đ' }}</span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                Giảm giá:
                                <span
                                    class="text-success sale">{{ number_format($discount, 0, ',', '.') . ' đ' }}</span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                Tổng cộng:
                                <span
                                    class="text-success totalPrice1">{{ number_format($totalPrice - $discount, 0, ',', '.') . ' đ' }}</span>
                            </li>
                        </ul>
                        <div class="d-flex justify-content-end">
                            <button class="btn bg-danger text-white m-3">
                                <a href="/order-bill" style="color:white;text-decoration: none">
                                    Tiếp Tục Thanh toán
                                </a>
                        </div>
                    </div>
...
document.querySelectorAll('.increaseQuantity').forEach(button => {
    button.addEventListener('click', function() {
        ...
        // Cập nhật tổng cộng
        totalPrice1Element.textContent = numberWithCommas((total - parseFloat(saleElement.textContent
            .replace(/\D/g, ''))).toFixed(0)) + ' đ';
    });
});
...
document.querySelectorAll('.decreaseQuantity').forEach(button => {
    button.addEventListener('click', function() {
        ...
        if (quantity > 1) {
            ...
            // Cập nhật tổng cộng
            totalPrice1Element.textContent = numberWithCommas((total - parseFloat(saleElement
                .textContent.replace(/\D/g, ''))).toFixed(0)) + ' đ';
        }
    });
});
